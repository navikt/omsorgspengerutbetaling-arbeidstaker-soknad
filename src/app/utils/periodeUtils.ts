...
export const getPeriodeBoundaries = (utbetalingsperiode: UtbetalingsperiodeApi[]): { min?: Date; max?: Date } => {
...
export const harFraværPgaSmittevernhensyn = (perioder: UtbetalingsperiodeApi[]): boolean => {
    return [...perioder].findIndex(({ årsak }) => årsak === FraværÅrsak.smittevernhensyn) >= 0;
};

export const harFraværPgaStengBhgSkole = (perioder: UtbetalingsperiodeApi[]): boolean => {
